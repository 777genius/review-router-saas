  ...
  it("runs the actual CLI with a real fake-gh executable", () => {
    ...
    writeFileSync(
      ...
      `#!${process.execPath}\n` +
        ...
        `const verificationResult=JSON.parse(fs.readFileSync("scripts/fixtures/live-catalog-attestation/gh-verification-result.json","utf8")).verificationResult;Object.assign(verificationResult,{statement,buildSignerURI:"https://github.com/"+workflow+"@refs/heads/main",runInvocationURI:"https://github.com/owner/repo/actions/runs/"+run+"/attempts/1"});const certificate=verificationResult.signature.certificate;certificate.subjectAlternativeName=verificationResult.buildSignerURI;certificate.extensions.buildSignerURI=verificationResult.buildSignerURI;\n` +
