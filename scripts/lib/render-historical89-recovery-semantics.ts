...
export function canonicalRecoveryCheck(definition: string): string {
  ...
  function conjunction(original: string): string[] | null {
    let s = original;
    for (;;) {
      let depth = 0, bracket = 0, quote = "", end = -1;
      const splits: number[] = [];
      for (let i = 0; i < s.length; i++) {
        const c = s[i]!;
        if (quote) {
          if (c === quote) { if (s[i + 1] === quote) i++; else quote = ""; }
          // Escape strings and uncommon syntax remain byte-exact.
          if (c === "\\") return null;
          continue;
        }
        if (c === "'" || c === '"') { quote = c; continue; }
        if (c === "$" || s.slice(i, i + 2) === "--" || s.slice(i, i + 2) === "/*") return null;
        if (c === "[") bracket++;
        if (c === "]") bracket--;
        if (c === "(") depth++;
        if (c === ")") { depth--; if (depth === 0 && end < 0) end = i; }
        if (depth < 0 || bracket < 0) return null;
        if (depth === 0 && bracket === 0 && s.slice(i, i + 5) === " AND ") splits.push(i);
      }
      if (quote || depth || bracket) return null;
      if (s[0] === "(" && end === s.length - 1) { s = s.slice(1, -1); continue; }
      if (!splits.length) return null;
      const parts: string[] = [];
      let start = 0;
      for (const at of splits) { parts.push(s.slice(start, at)); start = at + 5; }
      ...
      for (const part of parts) {
        ...
        for (let i = 0; i < part.length; i++) {
          const c = part[i]!;
          if (q) { if (c === q) { if (part[i + 1] === q) i++; else q = ""; } continue; }
          if (c === "'" || c === '"') q = c;
          else if (c === "(") d++;
...
export function recoverySemanticCommands(commands: CommandExecutor): CommandExecutor {
  return {
    execute(command, args, options) {
      const sql = args.at(-1) ?? "";
      const acl = command === "psql" && sql.includes("'kind','object'") && sql.includes("'acl',c.relacl");
      const check = command === "psql" && sql.includes("'kind','constraint'") && sql.includes("pg_get_constraintdef(c.oid)");
      const actual = acl ? [...args.slice(0, -1), sql.replace("'acl',c.relacl", `'acl',${relationAcl}`)] : args;
      const result = commands.execute(command, actual, options);
      if (!check) return result;
      const rows = JSON.parse(result.stdout);
      if (!Array.isArray(rows)) return result;
      return { ...result, stdout: JSON.stringify(rows.map(row => row.kind === "constraint" && typeof row.definition === "string"
        ? { ...row, definition: canonicalRecoveryCheck(row.definition) } : row)) };
    },
    hashStdout: (command, args, options) => commands.hashStdout(command, args, options),
    executeExpectingFailure: (command, args, options) => commands.executeExpectingFailure(command, args, options),
  };
}
