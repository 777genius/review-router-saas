...
import { canonicalRecoveryCheck as canonical, recoverySemanticCommands } from "./render-historical89-recovery-semantics";
...
function harness(stdout: string) {
  ...
  const commands: CommandExecutor = {
    execute(_command, args) { received = args; return { stdout }; },
    hashStdout: async () => { throw new Error("unexpected hash"); },
    executeExpectingFailure: () => { throw new Error("unexpected failure"); },
  };
  return { run: (sql: string) => recoverySemanticCommands(commands).execute("psql", ["--command", sql]), sql: () => received.at(-1)! };
}
...
describe("bounded recovery CHECK semantics", () => {
  it("equates both exact measured dump/reparse pairs", () => {
    for (const pair of measured.constraints_indexes_triggers) {
      ...
      expect(canonical(pair.left)).not.toBe(canonical(pair.right.replace('>= 1', '>= 2')));
    }
  });
  it("flattens only AND, with atoms and their order intact", () => {
    expect(canonical('CHECK (((a) AND (b)) AND ((c) AND (d)))')).toBe('CHECK ((a) AND (b) AND (c) AND (d))');
    const distinct = [
      ...
      ["CHECK ((x = 'a AND b'::text) AND (c))", "CHECK ((x = 'a OR b'::text) AND (c))"],
      ["CHECK ((x = 'a'' AND b'::text) AND (c))", "CHECK ((x = 'a'' AND b'::varchar) AND (c))"],
      ['CHECK (("AND" = 1) AND (c))', 'CHECK (("OR" = 1) AND (c))'],
      ['CHECK (((x + y) * z > 1) AND (c))', 'CHECK ((x + (y * z) > 1) AND (c))'],
    ];
    for (const [a, b] of distinct) expect(canonical(a!)).not.toBe(canonical(b!));
    for (const opaque of ['CHECK (a BETWEEN 1 AND 2)', 'CHECK (NOT ((a) AND (b)))', 'CHECK ((a) OR ((b) AND (c)))', 'CHECK ((f($q$ AND $q$)) AND (b))', 'CHECK ((a) AND (b)) NOT VALID']) expect(canonical(opaque)).toBe(opaque);
  });
  it("changes only constraint definitions in the actual command response", () => {
    const pair = measured.constraints_indexes_triggers[0]!;
    const rows = ['constraint', 'index', 'trigger'].map(kind => ({kind, name: 'any_name', definition: pair.left, other: 1}));
    const h = harness(JSON.stringify(rows));
    const result = JSON.parse(h.run("SELECT 'kind','constraint',pg_get_constraintdef(c.oid)").stdout);
    expect(result[0]).toEqual({...rows[0], definition: canonical(pair.right)});
...
describe("recovery relation ACL projection", () => {
  it("uses PostgreSQL default expansion for all eight measured owner/default pairs", () => {
    ...
    const h = harness('[]');
    h.run("SELECT 'kind','object','type',c.relkind,'owner',pg_get_userbyid(c.relowner),'acl',c.relacl,'default',d.defaclacl");
    expect(h.sql()).toContain("aclexplode(coalesce(c.relacl,acldefault(CASE WHEN c.relkind='S' THEN 's'::\"char\" ELSE 'r'::\"char\" END,c.relowner)))");
    expect(h.sql()).toContain("ELSE to_json(c.relacl) END");
    for (const fragment of ["'type',c.relkind", "'owner',pg_get_userbyid(c.relowner)", "'grantor',pg_get_userbyid(a.grantor)", "WHEN a.grantee=0 THEN 'PUBLIC'", "'granteePublic',a.grantee=0", "'privilege',a.privilege_type", "'grantable',a.is_grantable", "'default',d.defaclacl"]) expect(h.sql()).toContain(fragment);
  });
  it("does not collapse changed owner, grantor, grantee, PUBLIC, privilege or grant option", () => {
    const row = {kind:'object', type:'r', owner:'reviewrouter', acl:[{grantor:'reviewrouter', grantee:'reviewrouter', granteePublic:false, privilege:'SELECT', grantable:false}]};
    const variants = [{...row, owner:'other'}, {...row,type:'S'}, {...row,acl:[]}, ...[
      {grantor:'other'}, {grantee:'other'}, {grantee:'PUBLIC'}, {grantee:'PUBLIC',granteePublic:true}, {privilege:'UPDATE'}, {grantable:true},
    ].map(change => ({...row, acl:[{...row.acl[0], ...change}]}))];
    for (const variant of variants) {
      const h = harness(JSON.stringify([variant]));
      expect(JSON.parse(h.run("SELECT 'kind','object','acl',c.relacl").stdout)).toEqual([variant]);
...
describe("recovery semantic command boundary", () => {
  it("forwards other metadata, row hashes and denied-principal checks exactly", async () => {
    ...
    const commands: CommandExecutor = {
      execute(...args) { seen.push(args); return result; },
      async hashStdout(...args) { seen.push(args); return hash; },
      executeExpectingFailure(...args) { seen.push(args); return denied; },
    };
    const wrapper = recoverySemanticCommands(commands);
    const options = { timeoutMs: 1234 };
    for (const sql of ["SELECT 'notNull',a.attnotnull", "SELECT 'force',c.relforcerowsecurity", "SELECT 'kind','function'", "SELECT row_to_json(m)", "SELECT 'lastValue'", "SELECT effective_principals"]) {
