...
describe("trusted historical96 checkout reader", () => {
  it("validates the full97 source and returns only the exact immutable historical96", () => {
  ...
  it("also accepts a complete validated historical96 checkout", () => {
    reader.mockImplementationOnce(() => {
      partitionRenderSchemaHandoffCheckout(historical);
      return historical;
    });
  ...
  it("does not hide a rejected checkout-only SQL checksum", () => {
    reader.mockImplementationOnce(() => {
      const drifted = full.map((row, index) =>
        index === 96 ? { ...row, checksum: "a".repeat(64) } : row,
      );
