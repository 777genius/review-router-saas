...
export function persistentFixtureClient(
  pg: ReturnType<typeof managedPg17Fixture>, database: string, role: string,
) {
  ...
  return {
    ...
    async query(sql: string) {
      ...
      try {
        queries.push(sql);
        session.write(`\\set ON_ERROR_STOP off\n${sql}\n\\echo ${marker} :ERROR :SQLSTATE\n`);
        await waitFor(() => {
          if (session.closedResult()) throw new Error(session.stderr());
          return session.stdout().slice(offset).includes(`${marker} `) &&
            session.stdout().slice(offset).endsWith("\n");
        });
        const [raw, status] = session.stdout().slice(offset).split(`${marker} `);
        if (status.trim() !== "false 00000")
          throw new Error(`postgres:${status.trim()}:${session.stderr().slice(errorOffset)}`);
        const output = raw.trim();
        return { rows: output ? output.split("\n").map(text => {
    ...
    async end() {
      session.end("\\q\n");
      try { await session.result; } finally { await session.terminateAndWait(); }
    },
  };
}
