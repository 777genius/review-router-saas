...
`;
const expectedOfflinePrismaRun = `set -euo pipefail
    ...
    jobs: {
      producer: {
        ...
        steps: [
          ...
          {
            name: "Materialize exact pinned Prisma engines",
            shell: "bash",
            run: 'set -euo pipefail\ntest -z "${SUBSCRIPTION_RUNTIME_DEPLOY_KEY_B64:-}"\ntest -z "${GIT_SSH_COMMAND:-}"\ntest -z "${GIT_SSH_VARIANT:-}"\npnpm rebuild @prisma/engines\nengines_dir="$GITHUB_WORKSPACE/node_modules/.pnpm/@prisma+engines@7.8.0/node_modules/@prisma/engines"\ntest "$(node -p "require(\'$engines_dir/package.json\').version")" = "7.8.0"\ntest "$(node -p "require(\'$engines_dir/node_modules/@prisma/engines-version/package.json\').version")" = "7.8.0-6.3c6e192761c0362d496ed980de936e2f3cebcd3a"\ntest -x "$engines_dir/schema-engine-debian-openssl-3.0.x"\n',
          },
          {
            name: "Generate Prisma client offline after credential teardown",
            shell: "bash",
            run: expectedOfflinePrismaRun,
          },
          {
