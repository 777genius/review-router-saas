...
export type Request<K extends Operation> = Parameters<
  Handlers[K]["execute"]
>[0];
export type Response<K extends Operation> = Awaited<
  ReturnType<Handlers[K]["execute"]>
>;
export type Message =
  | {
      [K in Operation]: { id: string; operation: K; request: Request<K> };
    }[Operation]
...
export class OwnedControlPlane {
  ...
  constructor(
    ...
    spawn = () =>
      fork(
        fileURLToPath(
          new URL(
            "./investigation-control-plane-child.fixture.ts",
            import.meta.url,
          ),
        ),
        [],
    ...
    this.runId = runId;
    this.closeObserved = new Promise((resolve) => {
    ...
    const pendingPath = proofDirectory
      ? join(proofDirectory, randomUUID())
      : null;
    if (pendingPath)
      writeFileSync(pendingPath, runId, { flag: "wx", mode: 0o600 });
    this.child = spawn();
    this.child.on("spawn", () => {});
    this.child.on("error", () => this.rejectPending(fail("child_error")));
    this.child.on("exit", () => {
      this.exited = true;
      this.rejectPending(fail("child_exited"));
    });
  ...
  async start(config: FixtureConfig): Promise<Boot> {
    if (config.runId !== this.runId || this.boot)
      throw fail("ownership_mismatch");
    const boot = (await this.send(
      { operation: "configure", config },
      15_000,
    )) as Boot;
    if (
      boot.runId !== this.runId ||
      boot.pid !== this.child.pid ||
  ...
  private send(
    message: { operation: string; [key: string]: unknown },
    timeout = 10_000,
  ): Promise<unknown> {
    if (this.closed || this.exited || this.stopping || !this.child.connected)
      return Promise.reject(fail("child_unavailable"));
    const id = randomUUID();
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        this.pending.delete(id);
        reject(fail("request_timeout"));
      }, timeout);
      this.pending.set(id, {
        resolve: (value) => {
          clearTimeout(timer);
  ...
  async killAtCheckpoint(): Promise<void> {
    if (!this.boot || this.boot.runId !== this.runId)
      throw fail("ownership_unconfirmed");
    this.stopping = true;
    this.child.kill("SIGKILL");
    if (!(await this.waitForClose(5_000)))
      throw fail(`cleanup_incomplete_run_${this.runId}_pid_${this.child.pid}`);
  ...
  async close(): Promise<void> {
    ...
    if (this.child.connected)
      this.child.send({ id: randomUUID(), operation: "shutdown" }, () => {});
