...
function run(mode, url = 'postgresql://assigned:secret@127.0.0.1:6543/test', args = [], env = {}) {
...
`;
  ...
  try {
    const result = spawnSync('bash', [resolve('scripts/review-investigation-production-e2e/support/run-ci.fixture.sh'), ...args], {
      env: { ...process.env, PATH: `${dir}:${process.env.PATH}`, CI: 'true', MODE: mode, TEST_DIR: dir, TMPDIR: dir,
        REVIEW_ROUTER_TEST_DATABASE_URL: url, REAL_NODE: process.execPath, REVIEW_ROUTER_ITEM11_E2E: '', REVIEW_ROUTER_ITEM11_DATABASE_URL: '', REVIEW_ROUTER_ITEM11_RUN_ID: '', ...env }, encoding: 'utf8', timeout: 10000,
    });
    const read = name => { try { return readFileSync(join(dir, name), 'utf8'); } catch { return null; } };
    return { ...result, connection: read('connection'), dropped: read('dropped'), invocation: JSON.parse(read('invocation') ?? 'null') };
...
test('exact CI schema input is accepted, owned URL normalized, and item11 forced on', () => {
  const r = run('success', 'postgresql://postgres:postgres@localhost:5432/reviewrouter_test?schema=public');
  assert.equal(r.status, 0, r.stderr);
...
test('unsupported, duplicate, and nonpublic query parameters fail before provisioning', () => {
  for (const query of ['schema=private', 'sslmode=require', 'schema=public&schema=public', 'schema=public&x=1', 'x=1&schema=public']) {
    const r = run('success', 'postgresql://assigned:secret@localhost/test?' + query);
    assert.equal(r.status, 1);
    assert.equal(r.connection, null);
...
test('wrapper forwards filter, reporter, output and run options verbatim after one optional separator', () => {
  const args = ['--run', '-t', 'restores durable investigation state after OS process restart', '--reporter=json', '--outputFile=/tmp/report with spaces.json'];
  for (const prefix of [[], ['--']]) {
    const r = run('success', undefined, [...prefix, ...args], { CI: '' });
    assert.equal(r.status, 0, r.stderr);
    assert.deepEqual(r.invocation.args, ['exec', 'vitest', 'run', 'scripts/review-investigation-production-e2e/review-investigation-production.e2e.test.ts', ...args]);
    assert.equal(r.invocation.gate, '');
    assert.equal(r.connection, null);
  }
  const r = run('success', undefined, ['--', '--', '-t', 'literal'], { CI: '' });
...
test('local explicit gate fails closed before Vitest without matching assignment', () => {
  for (const assignment of [{}, { REVIEW_ROUTER_ITEM11_RUN_ID: 'a'.repeat(32) }, {
    REVIEW_ROUTER_ITEM11_RUN_ID: 'a'.repeat(32),
    REVIEW_ROUTER_ITEM11_DATABASE_URL: 'postgresql://assigned:secret@localhost/item11_test_' + 'b'.repeat(32),
  }]) {
    const r = run('success', undefined, [], { CI: '', REVIEW_ROUTER_ITEM11_E2E: '1', ...assignment });
    assert.equal(r.status, 1);
    assert.equal(r.invocation, null);
    assert.equal(r.connection, null);
  }
  const r = run('success', undefined, [], { CI: '', REVIEW_ROUTER_ITEM11_E2E: '1',
    REVIEW_ROUTER_ITEM11_RUN_ID: 'a'.repeat(32),
    REVIEW_ROUTER_ITEM11_DATABASE_URL: 'postgresql://assigned:secret@localhost/item11_test_' + 'a'.repeat(32) });
  assert.equal(r.status, 0, r.stderr);
  assert.equal(r.invocation.gate, '1');
  assert.equal(r.connection, null);
});
