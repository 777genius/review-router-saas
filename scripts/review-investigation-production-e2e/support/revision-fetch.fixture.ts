...
export function revisionFetch(fixture: RevisionFixture): typeof fetch {
  return async (input, init) => {
    ...
    if (
      request.method === "POST" &&
      url.pathname ===
        `/app/installations/${fixture.installationId}/access_tokens`
    ) {
      return json(201, {
    ...
    if (
      request.method === "GET" &&
      url.pathname === `${prefix}/pulls/${fixture.pullRequestNumber}`
    ) {
      return json(200, {
        number: fixture.pullRequestNumber,
        state: "open",
        base: {
          sha: fixture.revision.baseSha,
          repo: { full_name: `${fixture.owner}/${fixture.repo}` },
        },
    ...
    if (
      request.method === "GET" &&
      url.pathname ===
        `${prefix}/compare/${fixture.revision.baseSha}...${fixture.revision.headSha}`
    ) {
      return json(200, {
        merge_base_commit: { sha: fixture.revision.mergeBaseSha },
      });
    }
    throw new Error("item11_external_fetch_denied");
  };
