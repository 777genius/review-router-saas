...
function fixture() {
  ...
  const reply = (message, value) =>
    child.emit("message", { id: message.id, value });
  const ready = async () => {
    const start = owner.start({
      runId: owner.runId,
      databaseUrl: "generated-fixture",
      env: {},
    });
    reply(child.sent.at(-1), {
      pid: child.pid,
      nonce: "boot-a",
      runId: owner.runId,
      rss: 1,
...
test("exit rejects every pending request; null exitCode is not close evidence", async () => {
  ...
  const second = assert.rejects(
    f.owner.snapshot("investigation"),
    /child_exited/,
  );
...
test("SIGKILL checkpoint does not finish until the owned handle closes", async () => {
  ...
  await f.ready();
  let replaced = false;
  const killing = f.owner.killAtCheckpoint().then(() => {
    replaced = true;
  });
  f.child.emit("exit", null, "SIGKILL");
  await Promise.resolve();
...
test("startup errors retain the owned handle for cleanup", async () => {
  ...
  const starting = assert.rejects(
    f.owner.start({ runId: f.owner.runId, databaseUrl: "fixture", env: {} }),
    /child_error/,
  );
...
test("unknown termination fails bounded cleanup and retains ownership", async (t) => {
  ...
  const f = fixture();
  await f.ready();
  const killing = assert.rejects(
    f.owner.killAtCheckpoint(),
    /cleanup_incomplete_run_a+_pid_/,
  );
...
test("database cleanup ledger clears only on positive child close", async () => {
  ...
  try {
    ...
    if (previous === undefined)
      delete process.env.REVIEW_ROUTER_ITEM11_CHILD_PROOF_DIR;
