...
test("current-revision prerequisite receives exact parent fixture data", async () => {
  ...
  const pr = await (
    await fetch("https://api.github.com/repos/fixture/test/pulls/7")
  ).json();
  assert.equal(pr.head.sha, fixture.revision.headSha);
  assert.equal(pr.base.sha, fixture.revision.baseSha);
  const compare = await (
    await fetch(
      `https://api.github.com/repos/fixture/test/compare/${fixture.revision.baseSha}...${fixture.revision.headSha}`,
    )
  ).json();
...
test("unknown origins, methods, queries, identities and publication fail closed", async () => {
  ...
  await assert.rejects(
    fetch("https://api.github.com/repos/fixture/test/pulls/7", {
      method: "POST",
    }),
