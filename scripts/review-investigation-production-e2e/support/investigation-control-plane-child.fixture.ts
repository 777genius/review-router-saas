...
import type { Handlers, Message } from "./investigation-control-plane-process.fixture.js";
...
export async function durableSnapshot(prisma: PrismaClient, investigationId: string) {
  const investigation = await prisma.reviewInvestigation.findUniqueOrThrow({ where: { investigationId } });
  const where = { investigationId };
  const [obligations, turns, receipts, commands, leases, executionLeases, certificates, shadows, telemetry] = await Promise.all([
    prisma.reviewInvestigationObligation.findMany({ where, orderBy: { obligationId: "asc" } }),
    prisma.reviewInvestigationTurn.findMany({ where, orderBy: { turnId: "asc" } }),
    prisma.reviewInvestigationReceipt.findMany({ where, orderBy: { receiptId: "asc" } }),
    prisma.reviewInvestigationCommandReceipt.findMany({ where, orderBy: { commandId: "asc" } }),
    prisma.reviewInvestigationLease.findMany({ where, orderBy: { leaseId: "asc" } }),
    prisma.reviewInvocationLeaseV2.findMany({ where: { executionId: investigation.executionId }, orderBy: { leaseId: "asc" } }),
    prisma.reviewInvestigationCertificate.findMany({ where, orderBy: { certificateId: "asc" } }),
    prisma.reviewInvestigationShadowEvidence.findMany({ where, orderBy: { investigationId: "asc" } }),
    prisma.reviewInvestigationTelemetrySample.findMany({ where: { producerReleaseId: investigation.producerReleaseId, reviewRevisionHash: investigation.reviewRevisionHash }, orderBy: { sampleId: "asc" } }),
  ]);
...
export async function assertFixtureOwnership(prisma: PrismaClient, databaseUrl: string, runId: string) {
  ...
  if (!['localhost', '127.0.0.1', '[::1]'].includes(url.hostname) || url.pathname !== `/item11_test_${runId}` || url.search) {
    throw new Error("item11_database_not_assigned");
  }
  const rows = await prisma.$queryRaw<Array<{ run_id: string; database_name: string }>>`
    SELECT run_id, current_database() AS database_name FROM item11_fixture_owner`;
  if (rows.length !== 1 || rows[0]!.run_id !== runId || rows[0]!.database_name !== `item11_test_${runId}`) {
...
async function childMain() {
  ...
  process.on("message", (message: Message) => {
    void (async () => {
      ...
      try {
        if (message.operation === "shutdown") { await shutdown(); return; }
        ...
        if (message.operation === "configure") {
          ...
          handlers = {
            ...
            acquire: required(routes.investigation.acquireLease), release: required(routes.investigation.releaseLease),
            executionAcquire: required(routes.execution.acquireLease), executionRelease: required(routes.execution.releaseLease),
          };
          reply({ pid: process.pid, nonce: randomUUID(), runId: message.config.runId, rss: process.memoryUsage().rss });
          return;
        }
        ...
        switch (message.operation) {
          ...
          case "commit": result = await handlers.commit.execute(message.request); break;
          case "conclude": result = await handlers.conclude.execute(message.request); break;
          case "acquire": result = await handlers.acquire.execute(message.request); break;
          case "release": result = await handlers.release.execute(message.request); break;
          case "executionAcquire": result = await handlers.executionAcquire.execute(message.request); break;
