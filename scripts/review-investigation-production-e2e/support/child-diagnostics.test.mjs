...
import {
  childDiagnostic,
  formatChildDiagnostic,
} from "./child-diagnostics.fixture.ts";
...
test("diagnostics localize failures without transporting arbitrary exception text", () => {
  ...
  assert.equal(
    formatChildDiagnostic(childDiagnostic(error, "commit", "execute")),
    "child_operation_failed operation=commit phase=execute class=TypeError",
  );
  error.name = secret;
  assert.equal(
    formatChildDiagnostic(childDiagnostic(error, secret, "composition")),
...
test("allowlisted conflict remains recognizable and ownership failures identify phase", () => {
  assert.equal(
    formatChildDiagnostic(
      childDiagnostic(
        new Error("investigation_idempotency_conflict"),
        "commit",
        "execute",
      ),
    ),
  ...
  assert.equal(
    formatChildDiagnostic(
      childDiagnostic(
        new Error("item11_database_owner_mismatch"),
        "configure",
        "ownership",
      ),
    ),
