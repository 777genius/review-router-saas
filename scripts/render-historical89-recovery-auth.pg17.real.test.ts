...
(process.env.REVIEW_ROUTER_REQUIRE_HANDOFF_PG17 === "1"
  ? describe
  : describe.skip)("historical89 same-database credential retention, offline PG17.10", () => {
  ...
  const cluster = () => json(`SELECT jsonb_build_object(
  ...
  const relation = () => json(`SELECT jsonb_build_object(
  ...
  it("keeps TEST credentials and cluster principals while restoring data and ACLs into the existing database", async () => {
    ...
    expect(query("SELECT count(*) FROM public._prisma_migrations")).toBe("89");
    ...
    expect(query(`SELECT to_regclass('public."Workspace"') IS NULL`)).toBe("t");
    expect(cluster()).toEqual(before);
