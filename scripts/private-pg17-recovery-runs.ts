...
export async function discoverPrivatePg17RecoveryRuns(input: {
  ...
  const retentionCutoff = new Date(Date.now() - 91 * 24 * 60 * 60 * 1_000);
  const createdRange = `${retentionCutoff.toISOString().slice(0, 10)}..*`;
  for (let page = 1; page <= input.maximumPages; page += 1) {
    const response = await request(
      `https://api.github.com/repos/${input.repository}/actions/workflows/${encodeURIComponent(workflowFile)}/runs?event=workflow_dispatch&created=${encodeURIComponent(createdRange)}&per_page=100&page=${page}`,
      { headers },
    );
    if (!response.ok)
