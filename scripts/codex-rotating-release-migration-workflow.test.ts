...
describe("Codex rotating release migration workflow", () => {
  ...
  it("provisions custody against the full checkout catalog without exposing credentials", () => {
    expect(workflow).toContain(
    ...
    expect(canonicalPrismaMigrationCatalog).toEqual({
      appliedMigrationCount: 97,
      latestMigration: "000098_certified_fork_effect_archive",
    });
