...
BEGIN
  ...
  IF rollout.state <> 'pre_activation'
    ...
    OR intent.effect_state NOT IN ('dispatching','bound','cleaned','blocked')
  ...
  IF duplicate_job THEN
    UPDATE release_authority.runner_intent SET effect_state = 'blocked',
      effect_safe_for_compensation = false, effect_block_reason = 'duplicate',
      reconciliation_observation = jsonb_build_object(
        'reason', 'duplicate', 'lateProviderJobId', p_job->>'jobId'),
