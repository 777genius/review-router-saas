...
import {
  ...
  seedExecution,
  cleanup,
} from "../testing/prisma-investigation-store-harness";
  ...
  it("persists exactly one adoption receipt and rejects transactionally superseded execution", async () => {
    ...
    try {
      await harness.store.commit({
        investigation: seed,
        expectedVersion: null,
        commandId: `create-${seed.investigationId}`,
      ...
      const input = {
        ...
        commandId: `adopt-${seed.investigationId}`,
      ...
      expect(
        await harness.prisma.reviewInvestigation.findUniqueOrThrow({
          where: { investigationId: seed.investigationId },
        }),
      ).toEqual(before);
      ...
      await harness.prisma.reviewExecutionStreamV2.updateMany({
        where: { activeExecutionId: seed.executionId },
        data: { activeExecutionId: null },
      });
      const stale = { ...input, commandId: `stale-${seed.investigationId}` };
      await expect(harness.store.adopt(stale)).rejects.toThrow(
        "investigation_execution_superseded",
      );
