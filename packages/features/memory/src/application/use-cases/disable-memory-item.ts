...
export type DisableMemoryItemInput = {
...
export async function disableMemoryItem(
  ...
  if (existing.status === "disabled") {
    return { status: "noop", reason: "disabled", id: existing.id };
  }
  ...
  const item = MemoryItem.fromSnapshot(existing).disable({
    actor: input.actor,
    now,
  });
  ...
  try {
    return await dependencies.memoryTransaction.run(async (tx) => {
      ...
      await tx.memoryOutbox.enqueue({
        type: "memory.item.disabled",
        version: 1,
        idempotencyKey: `memory.item.disabled:${snapshot.workspaceId}:${snapshot.id}:${snapshot.version}`,
        workspaceId: input.workspaceId,
