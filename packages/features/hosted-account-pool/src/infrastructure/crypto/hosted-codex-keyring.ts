...
export function resolveHostedCodexKeyring(input: {
  ...
  if (input.env.NODE_ENV === "production") {
    ...
    if (input.externalKeyring && input.externalKeyring.custodyMode !== "aws_kms") {
      throw new Error("hosted_codex_external_kms_required");
    }
    if (input.externalKeyring) requireKeyArn(input.externalKeyring.currentKeyId);
    return input.externalKeyring ?? createProductionAwsKmsHostedCodexKeyring({
        env: input.env,
