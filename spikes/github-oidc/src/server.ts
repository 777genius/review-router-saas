...
import { createActionSessionToken, verifyActionSessionToken, verifyGitHubOidcToken } from './oidc.js';
...
const allowedRepositoryId = process.env.REVIEW_ROUTER_ALLOWED_REPOSITORY_ID || undefined;
const sessionSecret = process.env.REVIEW_ROUTER_ACTION_SESSION_SECRET || 'dev-only-review-router-spike-secret-at-least-32-bytes';
...
export function buildServer() {
  ...
  app.post('/api/action/v1/session/exchange', async (request, reply) => {
    ...
    const claims = await verifyGitHubOidcToken(token, audience, allowedRepositoryId);
  ...
  app.get('/api/action/v1/config', async (request, reply) => {
    ...
    if (!token) return reply.code(401).send({ error: { code: 'ACTION_SESSION_REQUIRED' } });
