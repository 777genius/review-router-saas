...
if (!owner || !repo) throw new Error('REVIEW_ROUTER_TARGET_REPO must be owner/repo');

const profile = loadAppProfile();
const app = createGitHubApp(profile);
const requestedSetupAuth = process.env.REVIEW_ROUTER_SETUP_AUTH === 'gh-user' ? 'gh-user' : 'app';
...
async function createSetupOctokit(setupAuth: SetupAuth): Promise<{ octokit: GitHubRequester; installationId: number | null }> {
  ...
  if (!installationId) {
    throw new Error(`ReviewRouter App is not installed on ${targetRepo}. Install ${profile.APP_NAME || profile.APP_SLUG || 'the app'} for this repository first.`);
  }
  ...
  return { octokit: await app.getInstallationOctokit(installationId), installationId };
}
...
async function createOrUpdateSetupPr(setupAuth: SetupAuth): Promise<void> {
  ...
  const branch = process.env.REVIEW_ROUTER_SETUP_BRANCH || 'reviewrouter/saas-spike';
  const path = '.github/workflows/reviewrouter-saas-spike.yml';
  const audience = process.env.REVIEW_ROUTER_OIDC_AUDIENCE || 'review-router-spike';
  ...
  await octokit.request('PUT /repos/{owner}/{repo}/contents/{path}', {
    ...
    content: Buffer.from(renderSpikeWorkflow({ audience, endpointUrl })).toString('base64'),
  });
  ...
  const pr = existingPrs.data[0] ?? (await octokit.request('POST /repos/{owner}/{repo}/pulls', {
    owner,
    repo,
...
try {
  ...
  if (requestedSetupAuth === 'app' && status === 403) {
    console.warn('GitHub App setup token lacks repository write/workflow permissions. Falling back to gh user token for setup PR.');
